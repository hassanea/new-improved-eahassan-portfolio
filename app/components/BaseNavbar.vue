<template>
  ...
  >
    ...
    <ul :id="navId" ref="trapRef" class="navbar-nav" :class="toggleMenuClass">
      <li v-for="{ name, path } in links" :key="name" ref="regLinks">
        <nuxt-link
          v-if="name !== 'Blog'"
          :to="path"
      ...
      <li v-for="{ name, to, target } in hashLinks" :key="name" ref="hashes">
        <nuxt-link
          :to="to"
          :tabindex="!toggle && !showMobileMenu ? null : -1"
          v-if="!target"
      ...
      <li v-if="!isTablet" ref="dropdown">
        <client-only>
          ...
          >
            <template #icon>
              <span class="dropdown-text sr-only md:not-sr-only">
                Settings
              </span>
            ...
            <template #default>
              ...
              >
                ...
                <lazy-base-button
                  ...
                  @keydown.prevent="handleLocaleChange(locale.code)"
                  :label="`Switch to ${locale.name}`"
                  v-tooltip.left="`${locale.name}`"
                  >{{ locale.name }}
                </lazy-base-button>
                ...
                <span>
                  ...
                  <lazy-nuxt-img
                    ...
                    height="20"
                  />
                  <lazy-font-awesome v-else icon="fa-solid fa-xmark" />
                </span>
              </div>
            </template>
          </base-drop-down-menu>
...
<script lang="ts" setup>
  ...
  //     currentLocale => currentLocale !== locale.value,
  //   );
  // });
