<template>
  <figure class="item-wrap relative overflow-hidden bg-white">
    <nuxt-img
    ...
    <figcaption>
      <div
        class="overlay"
        :style="portfolioFrameStyle"
        :class="portfolioFrameClass"
      >
...
<script setup>
  ...
  const portfolioFrameStyle = computed(() => {
    const frameImgURL = image(
      `/portfolio/overlay-bg.png`,
      {},
      { provider: 'imagekit' },
    );
    return { backgroundImage: `url(${frameImgURL})` };
  });

  const portfolioFrameClass = computed(() => {
    return { 'bg-repeat': portfolioFrameStyle.value };
  });
