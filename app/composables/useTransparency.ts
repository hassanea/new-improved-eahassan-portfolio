...
export const useTransparency = (
  ...
  onMounted(() => {
    const hasTransparentModeInStorage = getModeFromStorage(
      storageOption,
      'transparent-mode',
    );
    ...
    // @ts-ignore
    if (
      hasTransparentModeInStorage === fallback ||
  ...
  const setTransparentSwitchIcon = computed(() => {
    return transparentMode.value === 'reduce'
  ...
  const unWatchTransparency = watch(transparentMode, value => {
    return value === 'reduce'
