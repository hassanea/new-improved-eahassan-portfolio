...
export const useReducedMotion = (
  ...
  const motionToggle = useState('reduce-switch', () =>
    toValue(useMediaQuery('(prefers-reduced-motion: reduce)')),
  );

  onMounted(() => {
    const motionFromStorage = getModeFromStorage(
      storageOption,
      'reduced-motion',
    );
  ...
  const unWatchReduce = watch([reducedMotion, motionToggle], values => {
